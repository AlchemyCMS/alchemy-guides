...
Visit the Blog sample page and have a deep look at the
...
The individual page elements could be considered as an element.
...
#3 Afterwards create a new <code>app/assets/stylesheets/foundation_and_overrides</code> containing:
...
Hence he necessary prepations are completed.
...
The blog should have a unique title. The title elements contains a <code>title</code> and a <code>text</code> which represents a description or motto. In both cases we choose an EssenceText, due to the fact that the <code>title</code> and the <code>text</code>will contain simple text.

The <code>default</code> attribut provides the preset value, if the user didn't specify a motto or description.
...
The title is represented by <code>blog_post_title</code>. Its just a text, so we took EssenceText
as type. Below <code>settings</code> we use <code>linkable: true</code>, because the user should be able to choose a link for the title of the blog post.

The <code>blog_post_intro_text</code> text as well as the main content <code>blog_post_content</code>
get the type <code>EssenceRichtext</code>, since the user like to add multiline paragraphs.

The <code> blog_post_image</code> is of type EssencePicture. This will empower the user to add images.
...
The <code>categories_headline</code> is represented by an EssenceText.
...
*Its important to copy the definition of <code> category</code> below <code>contents</code> to <code>available_contents</code>*. Besides this, we also make the EssenceText deleteable and linkable via <code> settings</code>.
...
NOTE: Instead of EssenceText you could alternativly use EssenceLink for 'featured_link'.
