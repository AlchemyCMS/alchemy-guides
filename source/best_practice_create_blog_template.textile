...
* Integrate in your existing application
...
The individual page elements(see *1.*) could be considered as an element.

h4. Blog title

The blog should have a unique title. The title elements contains a <code>title</code> and a <code>text</code> which represents a description or motto. In both cases we choose an <code>EssenceText</code>, due to the fact that the <code>title</code> and the <code>text</code>will contain simple text.
