h2. Best Practice: How to create a contact form
...
on the website. A contact form is common and convenient way to enable them to contact you.
...
In contrast to "Best Practice: Create a Blog Template":best_practice_create_blog_template.html
...
First of all make sure, that the Alchemy mailer is correctly configured.
If a user submit the the form usually the user receives a confirmation mail and

This is the default mailer configuration (in <code>config/alchemy/config.yml</code>). Replace the placeholders for <code>mail_from:</code> amd <code>mail_to:</code> with your own email.
...
mailer:
  ...
  mail_from: contact@your-domain.com
...
Navigate to <code>app/views/alchemy/elements/_contactform_view.html.erb</code> and edit it with regard to the field names in <code>config/alchemy/config.yml</code>.
It should look like this:

<pre>
<%= form_for(@message ||= Alchemy::Message.new) do |form| %>
...
The rails helper <code>form_for()</code> is used to generate the form automatically.

NOTE: See the hidden field? This is important, or the messages mailer can't do all the magic for you. For further information: "MessageController documentation":http://www.rubydoc.info/github/magiclabs/alchemy_cms/Alchemy/MessagesController .
...
<pre>
<%= form.text_field ... %>
<%= form.text_field :firstname , label: _t(:sender, scope: 'my_forms.contactform') %>
<%= form.text_area ...  %>
</pre>

Your language yml should look like this: (<code>config/locales/en.yml</code>)
...
Now you can reuse the label by using the <code>_t</code> function while defining
<code>label</code>.
