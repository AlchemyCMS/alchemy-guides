...
Elements are customizable parts of your page. An article with headline, text block and a picture is an typical element.
...
* Defintion of an element
* Customization
* Integration into a page layout
...
* <code>name</code> <code>String</code> <strong>required</strong><br>
  A unique name of the element. This name is used in the <code>page_layouts.yml</code> file and in the <code>app/views/alchemy/elements</code> partials. The name itself is "translatable":#translations. The format has to be <strong>lowercase</strong> and seperated words has to be <strong>underscored</strong>.
...
In order to use it, you just need to enable the setting in the <code>elements.yml</code> file for every element that should contain a picture gallery.
...
INFO: The view generator <code>rails g alchemy:elements --skip</code> takes care of the partials. If you add the picture gallery editor to an existing element you have to insert <code>render_picture_gallery_editor(element)</code> into your elements editor partial.

h5. Code example of element editor partial (_picture_gallery_editor.html.haml)

h5. Code example of element editor partial (<code>_picture_gallery_editor.html.haml</code>)

<pre>
= render_picture_gallery_editor(element, :max_images => nil, image_size: "346x246", :crop => true, fixed_ratio: true)
</pre>

The counterpart in <strong>embedded Ruby (ERB)</strong> (<code>_picture_gallery_editor.html.erb</code>) might look like this:

<pre>
<%= render_picture_gallery_editor(element, image_size: "346x246", :max_images => nil , :crop => true, fixed_ratio: true) %>
</pre>
...
h5. The following code example shows how to render the element with the picture gallery (<code>_picture_gallery_view.html.haml</code>):
...
This code example illustrates the image cropping feature (including image caption):
...
If you are using <strong>ERB</strong> instead of <strong>Haml</strong> it might look like this:
...
It is important to define the dynamic content twice, below <code>contents:</code> and <code>available_contents:</code>

NOTE: It is necessary to define the contents which should be dynamic twice. In <code>contents</code> and <code>available_contents</code>.
...
Contents of type <code>EssenceText</code> and <code>EssenceRichtext</code> can be excluded from being indexed by the Ferret search engine.
...
h4. Preview for element editor

By default the header of an element editor (in the backend) shows the name of the element and the value of the first content.
If your first content is an <code>EssencePicture</code>, you would see the <strong>picture id</strong>. By using <code>take_me_for_preview</code>, you can define a more meaningful content (e. g. headline if exists):
...
In case of more than one content with <code>take_me_for_preview: true</code> option, the <strong>first detected content</strong> will be used.
...
This element is named <strong>article</strong>. It can be placed only once per page and it has three "contents":elements.html from type <code>EssencePicture</code>, <code>EssenceText</code> and <code>EssenceRichtext</code>.

h4. Assign elements to page layouts
...
You can now place the article element on every page which has the page layout named <strong>standard</strong>.
...
One file is named <code>_article_view.html.erb</code> and the second file is named <code>_article_editor.html.erb</code>.

# The element's view file  <code>_article_view.html.erb</code> gets rendered, when a user requests your webpage.
# The element's editor file <code>_article_editor.html.erb</code> gets rendered, when you edit the page in the Alchemy page editor.
...
For example, the the editor partial for the <strong>article</strong> element may look like this:
...
h4. Customizing the view partial
...
h4. Integrate elements in your page layout

Eventually you have to add your elements to your page layout.

Imagine you like to integrate your recently created <strong>article</strong> element as introduced above in your 'standard' page layout.
...
If you like to render all defined elements for a page layout you can just use the helper:
...
without any option.
<p>Otherwise if you want to prevent some elements from beeing displayed on a page:</p>

<pre>
<%= render_elements except: ['article', 'picture_gallery'] %>
</pre>

All elements which are defined for the specific page layout except <code>article</code> and <code>picture_gallery</code> will be displayed.
